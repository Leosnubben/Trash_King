...
public class Trashspawner : MonoBehaviour
{
    ...
    void Start()
    {
        plattformar = GameObject.FindGameObjectsWithTag("Ground");  //Säger att allt med "Ground" taggen är en plattform - Leo
    }
    IEnumerator TrashCreation() // Spawnar bara trash över plattformar - Leo
    {
    ...
    void Update()
    {
        if (trashcount <= AmountOfTrash) //Sätter en valfri gräns på hur mycket trash som kan spawnas - Leo
        {
            StartCoroutine(TrashCreation());
        }
        else if (trashcount >= AmountOfTrash) //Slutar generera trash om gränsen är nådd - Leo
        {
